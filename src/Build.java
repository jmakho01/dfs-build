...
public class Build {
  ...
  private static void printShortWords(Vertex<String> vertex, int k, Set<String> seen) {
    ...
    for(Vertex<String> neighbor : vertex.neighbors) { printShortWords(neighbor, k, seen); }
  }
  ...
  private static String longestWord(Vertex<String> vertex, Set<String> seen) {
    if(vertex == null || seen.contains(vertex.data)) return "";
    seen.add(vertex.data);
    String longest = vertex.data;
    for(Vertex<String> neighbor : vertex.neighbors) { 
      String longestNeighbor = longestWord(neighbor, seen);
  ...
  private static <T> void printSelfLoopers(Vertex<T> vertex, Set<T> seen) {
    if(vertex == null || seen.contains(vertex.data)) return;
    seen.add(vertex.data);
    for(Vertex<T> neighbor : vertex.neighbors) { 
      if(neighbor.data == vertex.data) { System.out.println(vertex.data); }
  ...
  public static boolean canReach(Airport start, Airport destination, Set<Airport> seen) {
    ...
    seen.add(start);
    for(Airport connecting : start.getOutboundFlights()) { if(canReach(connecting, destination, seen)) return true; }
    return false;
  }
  ...
  public static <T> Set<T> unreachable(Map<T, List<T>> graph, T starting) {
    Set<T> reached = unreachable(graph, starting, new HashSet<>());
    for(T vertex : reached){ if(graph.containsKey(vertex)) { graph.remove(vertex); } }
    return graph.keySet();
  }

  private static <T> Set<T> unreachable(Map<T, List<T>> graph, T starting, Set<T> seen) {
    if(starting == null || seen.contains(starting) || !graph.containsKey(starting)) return seen;
    seen.add(starting);
    for(T neighbor : graph.get(starting)) { unreachable(graph, neighbor, seen); }
    return seen;
  }
}
